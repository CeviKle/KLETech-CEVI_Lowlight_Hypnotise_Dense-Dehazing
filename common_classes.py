...
def run_test(model, dataloader_test, iteration, save_images, save_csv_files, metric_average_filename, mode, training=True):
    ...
    with torch.no_grad():
        ...
        for image_num, img in enumerate(dataloader_test):
            ...
            low = (np.clip(low[0].detach().cpu().numpy().transpose(1,2,0),0,1)*255).astype(np.uint8)
            ...
            if cond:
                ...
                cv2.imwrite(os.path.join(save_images,'{}_{}_psnr_{}_ssim_{}_C__input{}.jpg'.format(image_num,iteration, psnr_img, ssim_img,c_pred)), low)
